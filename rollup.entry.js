...
const generateConfig = (input) => ({
  ...
  output: {
    file: `./packages/cdk-construct/lib/microapps-app-nextjs-demo/${input.filename}${input.minify ? '' : '.max'}.mjs`,
    format: 'es',
    sourcemap: `./packages/cdk-construct/lib/microapps-app-nextjs-demo/${input.filename}${input.minify ? '' : '.max'}.mjs.map`,
  },
  plugins: [
    json(),
    commonjs(),
